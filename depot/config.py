...
class Config:
    ...
    def from_env(cls) -> "Config":
        ...
        # default is derived from them (the quarantine folder lives inside
        # the scan inbox's config subfolder, not under Dokumente/ - see
        # error_folder below).
        scan_eingang_webdav_path = os.environ.get(
            "SCAN_EINGANG_WEBDAV_PATH", "Dokumente/Scan Eingang"
        ).strip("/")
        config_subfolder = os.environ.get("CONFIG_SUBFOLDER", "Depot Config").strip("/")
        ...
        return cls(
            ...
            scan_eingang_webdav_path=scan_eingang_webdav_path,
            ...
            # Deliberately NOT under Dokumente/: _Fehlerhaft is DEPOT's own
            # quarantine bucket for documents that repeatedly fail
            # processing, not a real document a user would want to see
            # offered as a classification target. Lives next to the other
            # DEPOT-managed folders (config, logs, Processed) instead.
            error_folder=os.environ.get(
                "ERROR_FOLDER", f"{scan_eingang_webdav_path}/{config_subfolder}/_Fehlerhaft"
            ).strip("/"),
